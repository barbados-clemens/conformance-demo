...
function isBuiltinModuleImport(importExpr) {
  const packageName = (0,
  get_package_name_from_import_path_1.getPackageNameFromImportPath)(importExpr);
  return (
    (0, node_module_1.isBuiltin)(packageName) ||
    experimentalNodeModules.has(packageName)
  );
}
class TargetProjectLocator {
  constructor(
    nodes,
    externalNodes = {},
    npmResolutionCache = defaultNpmResolutionCache
  ) {
    this.nodes = nodes;
    this.externalNodes = externalNodes;
    this.npmResolutionCache = npmResolutionCache;
    this.projectRootMappings = (0,
    find_project_for_path_1.createProjectRootMappings)(this.nodes);
  ...
  findNpmProjectFromImport(importExpr, fromFilePath) {
    const packageName = (0,
    get_package_name_from_import_path_1.getPackageNameFromImportPath)(
      importExpr
    );
    ...
    if (fromFilePath.startsWith(workspace_root_1.workspaceRoot)) {
      workspaceRelativeFilePath = fromFilePath.replace(
        workspace_root_1.workspaceRoot,
        ''
      );
    } else {
      fullFilePath = (0, node_path_1.join)(
        workspace_root_1.workspaceRoot,
        fromFilePath
      );
    }
    const fullDirPath = (0, node_path_1.dirname)(fullFilePath);
    const workspaceRelativeDirPath = (0, node_path_1.dirname)(
  ...
  findImportInWorkspaceProjects(importPath) {
    this.packagesMetadata ??= (0, packages_1.getWorkspacePackagesMetadata)(
      this.nodes
    );
    ...
    const project = (0,
    packages_1.matchImportToWildcardEntryPointsToProjectMap)(
  ...
  resolveImportWithTypescript(normalizedImportExpr, filePath) {
    ...
    if (this.typescriptResolutionCache.has(normalizedImportExpr)) {
      ...
      resolvedModule = (0, typescript_1.resolveModuleByImport)(
  ...
  resolveImportWithRequire(normalizedImportExpr, filePath) {
    return node_path_1.posix.relative(
      workspace_root_1.workspaceRoot,
  ...
  findMatchingProjectFiles(file) {
    const project = (0, find_project_for_path_1.findProjectForPath)(
      file,
  ...
  readPackageJson(packageName, relativeToDir) {
    ...
    const packageJsonPath = (0, resolve_relative_to_dir_1.resolveRelativeToDir)(
      (0, node_path_1.join)(packageName, 'package.json'),
      relativeToDir
    );
    if (packageJsonPath) {
    ...
    try {
      ...
      const pathOfFileInPackage =
        packageJsonPath ??
        (0, resolve_relative_to_dir_1.resolveRelativeToDir)(
          packageName,
          relativeToDir
        );
      ...
      while (dir !== (0, node_path_1.dirname)(dir)) {
        ...
        try {
          const parsedPackageJson = (0, fileutils_1.readJsonFile)(
            packageJsonPath
          );
